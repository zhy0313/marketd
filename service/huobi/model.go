...
type KLineData struct {
	ID     uint64  `json:"id"`     // K线ID
	Amount float64 `json:"amount"` // 成交量
	Count  uint64  `json:"count"`  // 成交笔数
...
type KLineReturn struct {
	...
	Ts      uint64      `json:"ts"`       // 响应生成时间点, 单位毫秒
...
type MarketDepthReturn struct {
	...
	Ts      uint64      `json:"ts"`     // 响应生成时间点, 单位: 毫秒
...
type TradeData struct {
	...
	Ts        uint64      `json:"ts"`        // 成交时间
}

type TradeTick struct {
	ID   uint64      `json:"id"`   // 消息ID
	Ts   uint64      `json:"ts"`   // 最新成交时间
